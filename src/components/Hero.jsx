...
import { ArrowUpRight, ShieldCheck, Trees, Home, Sparkles, Shield, Building2, MapPin } from 'lucide-react';
...
export default function Hero({ onOpenBookVisit }) {
  ...
  useEffect(() => {
    // Check if video is already ready/playing
    if (videoRef.current && videoRef.current.readyState >= 3) {
      setIsVideoLoaded(true);
    }
  }, []);

  // 4 Key Button / Badge Details for the first section of the main page
  const keyHighlights = [
    {
      icon: Trees,
      title: '55 Acres',
  ...
  return (
    <section id="home" className="hero-section" aria-label="Maytri Ambhuja Hero">
      ...
      <div className="hero-media-wrapper">
        {/* Luxury animated loading indicator while video loads */}
        {!isVideoLoaded && (
        ...
        <video
          ...
          src={CLOUDINARY_MEDIA.heroVideo}
          autoPlay
          loop
          muted
          playsInline
      ...
      <div className="hero-container">
        <div className="hero-content">
          ...
          <div className="hero-eyebrow">
            <span className="eyebrow-pill">MAYTRI GROUP &amp; SANGHI CITY</span>
            <span className="eyebrow-rera">
              <ShieldCheck size={14} className="eyebrow-rera-icon" />
              RERA NO: P02400007647
            </span>
          </div>

          {/* Main Title */}
          <h1 className="hero-title">
            Exclusive Premium Villa Township near <br className="hidden md:inline" />
            ORR, EXIT no 11.
          </h1>
          ...
          <div className="hero-price-badge">
            <span className="hero-price-highlight">
              Spacious Villa Starts from 3.2Cr + Amenities Extra
            </span>
          </div>

          {/* Supporting Description */}
          <p className="hero-description">
            Experience ultra-luxury living spread across a magnificent 55-acre master-planned township with 516 bespoke triplex villas and Hyderabad's grandest 90,000 sq.ft clubhouse.
          </p>
